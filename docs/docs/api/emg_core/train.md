...
```python
def train_model(user_id: str, model_type: str = 'rf', test_size: float = 0.2, config_obj: Optional[TrainingConfig] = None) -> Dict[str, Any]
```
...
```python
def calibrate_model(user_id: str, pretrained_model_type: str, calibration_config: Optional[TrainingConfig] = None) -> Dict[str, Any]
```
